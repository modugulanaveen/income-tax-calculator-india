...
export default function AddEmployee({
  ...
  const calculate = () => {
    const gross = (formData.earnings || []).reduce(
      (sum, e) => sum + (parseFloat(e.amount) || 0),
      0,
    );
    const deductions = (formData.deductions || []).reduce(
  ...
  const validateSalaryData = () => {
    ...
    if (deductions > gross * 0.75) {
      warnings.push(
        "⚠️ Deductions are too high (>75% of gross). Please verify.",
      );
    }
    if (deductions > gross) {
      warnings.push(
        "❌ ERROR: Deductions exceed gross earnings! This will result in negative net salary.",
      );
    }
    if (gross > 5000000) {
      warnings.push(
        "⚠️ Gross earnings is very high (>₹50 lakhs). Please verify.",
      );
    }
  ...
  const updateEarning = (idx, key, value) =>
    setFormData((prev) => ({
      ...prev,
      earnings: (prev.earnings || []).map((it, i) =>
  ...
  const updateDeduction = (idx, key, value) =>
    setFormData((prev) => ({
      ...prev,
      deductions: (prev.deductions || []).map((it, i) =>
        i === idx ? { ...it, [key]: value } : it,
      ),
    }));
  ...
  const handleImportFromCSV = (csvData) => {
    if (csvData && csvData.length > 0) {
      ...
      setFormData((prev) => ({
        ...
        payPeriod:
          firstEmployee.payPeriod || new Date().toISOString().slice(0, 7),
        ...
        deductions: firstEmployee.deductions || [
          { label: "Income Tax", amount: "0" },
        ],
      }));
    }
    ...
    <div className="add-employee-page">
      ...
      <div className="top-section">
        ...
        <div className="pay-month-section">
          <div className="pay-month-title">Payslip For the Month</div>
            ...
            <div className="summary-row">
              ...
              <div className="input-field">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setFormData({ ...formData, lossOfPayDays: e.target.value })
                  }
                />
              </div>
            </div>
        ...
        <div className="income-table">
                ...
                <div className="table-row" key={`e${i}`}>
                  <div className="cell">
                    <input
                      ...
                      value={earning.label}
                      onChange={(e) =>
                        updateEarning(i, "label", e.target.value)
                      }
                  ...
                  <div className="cell amount-cell">
                    <input
                      ...
                      onChange={(e) =>
                        updateEarning(i, "amount", e.target.value)
                      }
                      placeholder="0"
                    />
                  </div>
          ...
          <div className="table-footer">
            ...
            <div className="footer-right">
              Total Deductions{" "}
              <strong>₹{preview.deductions.toLocaleString()}</strong>
            </div>
          </div>
